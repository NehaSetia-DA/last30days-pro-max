...
Each run's evidence is stored so the report's "What Changed This Month" section
can compare the current run against the most recent prior run on the same topic
and label evidence as new, recurring, fading, or rank-changed. Identity across
runs is the result URL. Uses the stdlib ``sqlite3`` only.
"""
    ...
    *,
    ...
    conn.executemany(
        """INSERT INTO evidence(
            ...
            competitor, source_type, serp_rank, content_hash
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        [
            (
...
def compute_deltas(
    ...
    *,
    ...
    Must be called BEFORE ``save_run`` for the current run, so the "previous"
    run is genuinely the one before this one.
    """
    ...
    # Collapse current evidence to one row per URL, keeping the best (lowest) rank.
