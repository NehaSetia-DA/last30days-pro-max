...
stdlib only (no MCP SDK dependency, per the project's stdlib-first rule). It
handles ``initialize``, ``tools/list``, and ``tools/call`` against the tool
registry in :mod:`last30days_pro_max.tools`.
...
SERVER_INFO = {"name": "last30days-pro-max", "version": __version__}
...
def handle_message(message: dict):
    ...
    # Unknown notifications (no id) are ignored; unknown requests get an error.
